...
import { func, object, string, bool, any, array, oneOfType, number } from 'prop-types'
...
export function getSuggestionLabel(suggestion, mainField) {
    return suggestion?.[mainField] ?? suggestion?.name ?? suggestion?.label ?? ''
}
    ...
    useEffect(() => {
        if (suggestions && suggestions.length > 0) {
            ...
            suggestions?.forEach(suggestion => {
                ...
                if (
                    (value ? toCompare && comparisonFunction(toCompare, value) : true) &&
                    tmp.findIndex(sugg => getSuggestionLabel(sugg, mainField) === toCompare) === -1
                ) {
                    tmp.push(suggestion)
                }
            })
...
export const InputAutocomplete = ({
    ...
    return (
        <Controller
            ...
            name={name}
            rules={rules}
            defaultValue={defaultFound instanceof Object ? defaultFound?.id : defaultValue}
            render={({ field: { onChange, value, name } = {} }) => {
                const found =
                    suggestions?.length > 0 && suggestions?.find(sugg => sugg?.id === value)

                return (
                    <Autocomplete
                        {...otherProps}
                        value={
                            found instanceof Object ? getSuggestionLabel(found, mainField) : value
                        }
                        ...
                        onChange={valueOrObj => {
                            const newFound = suggestions?.find(
                                sugg =>
                                    ...
                                    getSuggestionLabel(sugg, mainField) === valueOrObj,
                            )

                            if (newFound instanceof Object) {
                                return onChange(
                                    newFound?.id ?? getSuggestionLabel(newFound, mainField) ?? '',
                                )
                            }
