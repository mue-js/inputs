...
export const Datepicker = ({
    ...
    return (
        ...
        >
            ...
            >
                {({ close }) => <Calendar date={value} onChange={onChange} close={close} />}
            </Dropdown>

            <div className="flex justify-space-between">
                <div className="helper flex-1">{error ? error ?? 'Erreur' : helper}</div>
            </div>
        </label>
    )
}
...
export const InputDate = ({ control, name, rules, defaultValue, ...otherProps }) => {
    return (
        <Controller
            control={control}
            name={name}
            rules={{
                ...
                validate: date =>
                    date &&
                    ((typeof rules?.validate === 'function' && rules.validate(date)) ||
